...
impl<T, U> Terms<T, PauliCode<U>> for OneElectron<U>
where
    T: Float,
    U: Idx,
{
    fn add_to(
        ...
        if p == q {
            repr.add(&PauliCode::Identity, half);
            repr.add(&PauliCode::with_pauli(q, Pauli::Z), -half);
        } else {
...
pub struct TwoElectron<U> {
    orbs: ((Orbital<U>, Orbital<U>), (Orbital<U>, Orbital<U>)),
}

impl<U> TwoElectron<U>
where
    U: Idx,
{
    pub fn new(
...
impl<T, U> Terms<T, PauliCode<U>> for TwoElectron<U>
where
    T: Float,
    U: Idx,
{
    fn add_to(
        ...
        if p == s && q == r {
            ...
            repr.add(&PauliCode::IDENTITY, quarter);
            repr.add(&PauliCode::with_pauli(p, Pauli::Z), -quarter);
            repr.add(&PauliCode::with_pauli(q, Pauli::Z), -quarter);
            repr.add(&PauliCode::from([(p, Pauli::Z), (q, Pauli::Z)]), quarter);
        } else if q == r {
            let quarter = T::from(0.25).unwrap();
            let mut code = PauliCode::from(
...
impl<T, U> Terms<T, PauliCode<U>> for MullikenCode<U>
where
    T: Float,
    U: Idx,
{
        ...
        match self {
            MullikenCode::One(i, j) => {
                for spin in [Spin::Down, Spin::Up] {
                    ...
                    let mut code = OneElectron::new(p, q).unwrap();
                    code.add_to(repr);
                }
            }
